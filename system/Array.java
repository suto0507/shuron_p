...
public class Array {
	...
	Expr index_access_array(Expr ref_expr, IntExpr index, Check_status cs){
		return cs.ctx.mkSelect(cs.ctx.mkSelect(array, ref_expr), index);
	}
	
	Expr get_array(Expr ref_expr, IntExpr index, Check_status cs){
		return cs.ctx.mkSelect(array, ref_expr);
	}
	
	//配列自体を丸ごと更新
	void update_array(Expr ref_expr, Expr value, Check_status cs) throws Exception{
		String ret = "Array" + "_" + cs.Check_status_share.get_tmp_num();
	...
	//配列の中身の、特定の要素だけ更新
	void update_array(Expr ref_expr, IntExpr index, Expr value, Check_status cs) throws Exception{
		String ret = "Array" + "_" + cs.Check_status_share.get_tmp_num();
		Expr expr = cs.ctx.mkArrayConst(ret, cs.ctx.mkUninterpretedSort("ArrayRef"), cs.ctx.mkArraySort(cs.ctx.mkIntSort(), cs.ctx.mkUninterpretedSort("Ref")));
